...
import {
  SMARTCHAIN_MAINNET_CHAIN_ID,
  SMARTCHAIN_MAINNET_URL,
  web3Modal,
} from "../constants";
...
const useWeb3 = () => {
  ...
  const switchChain = async (web3Instance: any) => {
    try {
      ...
      await web3Instance.request({
        ...
        params: [
          {
            chainId: SMARTCHAIN_MAINNET_CHAIN_ID,
            rpcUrls: [SMARTCHAIN_MAINNET_URL],
      ...
      await web3Instance.request({
        ...
        params: [{ chainId: SMARTCHAIN_MAINNET_CHAIN_ID }], // chainId must be in hexadecimal numbers
      });
      return true;
    } catch (err: any) {
      console.log(err);
  ...
  const handleChangeChain = async (chainId: any) => {
    ...
    if (chainId !== SMARTCHAIN_MAINNET_CHAIN_ID) {
